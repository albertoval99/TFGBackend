...
router.post(
    ...
    async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            res.status(500).json({ message: "Error al crear el entrenador" });
        }
    }
);
...
router.post(
    ...
    async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            res.status(201).json({arbitro: arbitroRegistrado});
        } catch (error) {
            console.error("❌ Error al crear el árbitro:", error);
            res.status(500).json({ message: "Error al crear el árbitro" });
        }
    });
...
router.post(
    ...
    async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            res.status(201).json({jugador: jugadorRegistrado});
        } catch (error) {
...
router.get("/arbitro/:id_usuario", async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        res.status(500).json({message: "Error al obtener arbitro"});
    }
});
...
router.put("/jugador/:id_jugador", esAutorizado, esEntrenador, async (req: Request, res: Response) => {
    try {
        ...
        let numeroCamisetaNum;
        if (numero_camiseta == null) {
            numeroCamisetaNum = undefined;
        } else {
            numeroCamisetaNum = Number.parseInt(numero_camiseta);
        }
        let activoBool;
        if (activo == null) {
            activoBool = undefined;
        } else {
